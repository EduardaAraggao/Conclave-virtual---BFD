...
function verificarResultado(cardeais, votos) {
  ...
  if (eleito) {
    console.log(
      `\n O novo Papa eleito é: ${eleito.nome} com ${votos[eleito.id - 1]} votos!`
    );
    return true;
  } else {
    console.log(
      "\n Nenhum cardeal alcançou 2/3 dos votos. Haverá nova votação!"
    );
    return false;
  }
}
...
function conclaveVirtual() {
  ...
  while (!eleito) {
    ...
    if (!eleito) {
      let escolha = prompt("\nDeseja continuar a votação? (S/N): ").toUpperCase();
      if (escolha !== "S") {
